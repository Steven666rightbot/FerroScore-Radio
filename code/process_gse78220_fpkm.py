#!/usr/bin/env python3
"""
Process GSE78220 FPKM data and calculate FerroScore-Immuno
"""
...
print("=" * 60)
print("Processing GSE78220 FPKM Data")
print("=" * 60)

# Load FPKM data
fpkm_file = f'{EXTERNAL_DIR}/GSE78220_PatientFPKM.xlsx'
print(f"\nLoading: {fpkm_file}")

# Read Excel file
try:
    fpkm_data = pd.read_excel(fpkm_file)
    print(f"  FPKM data shape: {fpkm_data.shape}")
    print(f"  Columns: {list(fpkm_data.columns[:5])}...")
    print(f"  First few rows:")
    print(fpkm_data.head())
except Exception as e:
    print(f"  [X] Error reading Excel: {e}")
    exit(1)
...
expr_matrix.to_csv(f'{RESULTS_DIR}/gse78220_fpkm_extracted.csv')
print(f"  [OK] Saved: gse78220_fpkm_extracted.csv")
...
# Save scores
scores_df.to_csv(f'{RESULTS_DIR}/gse78220_ferro_immuno_scores.csv')
print(f"\n  [OK] Saved: gse78220_ferro_immuno_scores.csv")

print("\n" + "=" * 60)
print("GSE78220 Processing Complete!")
print("=" * 60)
print("\nNext: Validate model predictions against actual response")
